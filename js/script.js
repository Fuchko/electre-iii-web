function createForm(config = null) {
    ...
    for (let i = 0; i < numAlternatives; i++) {
        ...
        altDiv.innerHTML += `
            <div class="mb-2">
                Альтернатива ${i + 1}: <input type="text" class="form-control d-inline-block w-auto" id="alt_${i}" value="${altValue}" oninput="updateAltName(${i})">
            </div>
        `;
    }
    ...
    for (let j = 0; j < numCriteria; j++) {
        ...
        critDiv.innerHTML += `
            <div class="mb-2">
                Критерій ${j + 1}: 
                <input type="text" class="form-control d-inline-block w-auto mb-1" id="crit_${j}" value="${crit ? crit.name : 'Критерій' + (j + 1)}">
                Вага: <input type="number" class="form-control d-inline-block w-auto mb-1" id="weight_${j}" step="0.01" value="${crit ? crit.weight : 0.5}">
                q: <input type="number" class="form-control d-inline-block w-auto mb-1" id="q_${j}" value="${crit ? crit.q : 1}">
                p: <input type="number" class="form-control d-inline-block w-auto mb-1" id="p_${j}" value="${crit ? crit.p : 5}">
                v: <input type="number" class="form-control d-inline-block w-auto mb-1" id="v_${j}" value="${crit ? crit.v : 10}">
            </div>
        `;
    }
...
function saveConfig() {
    ...
    const alternatives = [];
    for (let i = 0; i < numAlternatives; i++) {
        alternatives.push(document.getElementById(`alt_${i}`).value.trim());
    }

    const criteria = [];
    for (let j = 0; j < numCriteria; j++) {
        criteria.push({
            name: document.getElementById(`crit_${j}`).value.trim(),
            weight: parseFloat(document.getElementById(`weight_${j}`).value),
            q: parseFloat(document.getElementById(`q_${j}`).value),
            p: parseFloat(document.getElementById(`p_${j}`).value),
            v: parseFloat(document.getElementById(`v_${j}`).value),
        });
    }
    ...
    for (let i = 0; i < numAlternatives; i++) {
        const row = [];
        for (let j = 0; j < numCriteria; j++) {
            row.push(parseFloat(document.getElementById(`eval_${i}_${j}`).value));
        }
        evaluations.push(row);
    }
    ...
    a.download = 'electre_config.json';
    a.click();
}
...
function loadConfig(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            document.getElementById('numAlternatives').value = config.alternatives.length;
            document.getElementById('numCriteria').value = config.criteria.length;
            createForm(config);
            showMessage('Конфігурацію завантажено успішно.', 'success');
        } catch (err) {
            showMessage('Помилка при завантаженні конфігурації.', 'error');
        }
    };
